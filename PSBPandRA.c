							...
							void *callbackData, int eventData1, int eventData2)
{
	...
	*/ 
	...
	{
		case EVENT_COMMIT:
			if(control == PANEL_4_RB_TCRA){
				GetCtrlVal(myApp.hControl,PANEL_4_RB_TCRA, &control_value);
				ToggleCntrFn(control, control_value);
			} else if(control == PANEL_4_RB_TCBA){
...
int ToggleCntrFn (int control, int control_value) {
	...
	if(control == PANEL_4_RB_TCBA){
		...
		***************************************/
		...
		/*****************************************************************************************/
	}else if(control == PANEL_4_RB_TCRA) { 
		
		if (control_value) {
			value_V = myApp.appCal.calRAControl.RA_Voltage;
			value_I = myApp.appCal.calRAControl.RA_Current;
		...
		/****************************************************************************************
		* 	Generate TC Red Act signal  "Dev1/Ctr0" 
		*		
		*		
		*
		***************************************/
...
int AO_BAVoltFn(double voltBA)
{
	...
	******/
	...
	if (volt_Test != 0.0){
		myApp.appCal.calBAControl.BA_Voltage = voltBA;
...
int AO_RAV_I_Fn(double v_RA, double i_RA)
{
	...
	******/
	...
	if (v_Test > maxV)
	{
		sprintf(strTemp,"Voltage value is out of bounds for Red Actinic RA.%f\n \n So nothing done\n", v_Test);
		MessagePopup("RA voltage To High",strTemp);
		return 0;
	}
	
	if ((v_Test < minV) && (v_Test != 0.0))
	{
		sprintf(strTemp,"Voltage value is out of bounds for Red Actinic RA.%f\n \n So nothing done\n", v_Test);
	...
	if (i_Test > maxI)
	{
		sprintf(strTemp,"Current value is out of bounds for Red Actinic RA.%f\n \n So nothing done\n", i_Test);
		MessagePopup("BA current To High",strTemp);
		return 0;
	}
	
	if ((i_Test < minI) && (i_Test != 0.0))
	{
		sprintf(strTemp,"Current value is out of bounds for Red Actinic RA.%f\n \n So nothing done\n", i_Test);
...
int RA_Scan_IFn(double min, double max)
{
	...
	****/
	...
	plotHandle = PlotX(myApp.hCalibration,PANEL1_GRAPH,dataX_I,(size_t)iNumPts, VAL_DOUBLE,VAL_SCATTER,VAL_EMPTY_SQUARE,VAL_SOLID,
			  1,VAL_YELLOW);
...
int AO_BPV_I_Fn(double v_BP, double i_BP)
{
	...
	***********/
	if (v_Test > maxV)
	{
		sprintf(strTemp,"Voltage value is out of bounds for Blue Pulse (BP).%d\n \n So nothing done\n", v_Test);
		MessagePopup("BP voltage Too High",strTemp);
		return 0;
	}
	
	if ((v_Test < minV) && (i_Test != 0.0))
	{
		sprintf(strTemp,"Voltage value is out of bounds for Blue Pulse (BP).%f\n \n So nothing done\n", v_Test);
	...
	if (i_Test > maxI )
	{
		sprintf(strTemp,"Current value is out of bounds for Blue Pulse (BP).%f\n \n So nothing done\n", i_Test);
...
int BP_Scan_IFn(double min, double max)
{
	...
	********************/
	...
	// Make bounds local
	//min = myApp.appCal.calBPControl.iBP_Low;
	//max = myApp.appCal.calBPControl.iBP_High;
	double dMinI = min;
	double dMaxI = max;
	...
	****/
	...
	// Set up XAxis
	// set graph controls for a gain of 1 on the X axis
	SetCtrlAttribute(myApp.hCalibration,PANEL1_GRAPH,ATTR_XAXIS_GAIN,1.0);
	
	log = (int)log10(rate);
	SetCtrlAttribute(myApp.hCalibration,PANEL1_GRAPH,ATTR_XPRECISION,2);
