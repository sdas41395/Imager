...
static int CVIWndHndl;

static int g_hchild1;
int hParent;
int g_hchild2;
int g_hchild3;
...
int main (int argc, char *argv[])
{
	...
	// errChk (g_hchild3 = LoadPanel (hParent, "FlourImager.uir", PANEL_2);
   
    if (hParent   < 0
    ||  g_hchild1 < 0 || g_hchild2 < 0)
		return -1;
	...
	SetAxisScalingMode (g_hchild2, PANEL_GRAPH, VAL_LEFT_YAXIS,
...
static void GenSquareWaveRA(int numElements, double ampI, double ampV, double squareWave[], double temp1[], double temp2[])
{
							   ...
							   void *callbackData, int eventData1, int eventData2)
{
	...
	int         waveformType;
	double      min,max,frequency,resultingFrequency,amplitude,desiredSampClkRate,resultingSampClkRate;
	uInt32		sampsPerCycle,TotalsampsPerCycle;
	uInt32      sampsPerBuffer;
	uInt32 		numChannels=0;
	float64     *data=NULL;
	...
	if( event==EVENT_COMMIT ) {
		...
		GetCtrlVal (panel, PANEL1_RAONOFF, &raOnOff);
							...
							void *callbackData, int eventData1, int eventData2)
{
	...
	int         error=0;
	TaskHandle  taskHandle1=0,taskHandle2=0;
	TaskHandle	taskHandle5=0;
	TaskHandle 	gTaskHandle=0,gTaskHandle1=0;
	uInt8	    dataD[8] = {0};
	...
	if( event==EVENT_COMMIT ) { 
		...
		****/
		...
		//Get number of channels from task
		DAQmxErrChk (DAQmxGetTaskAttribute(gTaskHandle1,DAQmx_Task_NumChans,&numChannels));
		
		// Calculate total samples since each waveforem is 1 cycle: sampsPerCycle equals number of finite outputs
		TotalsampsPerCycle = numChannels*sampsPerCycle;
		DAQmxErrChk (DAQmxCfgSampClkTiming (gTaskHandle1, "", rate, DAQmx_Val_Rising, DAQmx_Val_FiniteSamps, sampsPerCycle));
		...
		SetCtrlAttribute(panel,PANEL1_GRAPH,ATTR_XAXIS_GAIN,1/(rate - 20));
		log = (int)log10(rate-20);
		SetCtrlAttribute(panel,PANEL1_GRAPH,ATTR_XPRECISION,log); 
