...
int Cam_Picture()
{
	...
	*/
	...
	// the name should be unique so part of it would be time stamp or index number
	char name[50];
	
	// the sequence number should be the first cell in the row for the struct
	...
	// ********* should be from CExperiment struct in array not local myData 
	char TimeStamp[20];
	sprintf(TimeStamp, "%d,%d,%d\\%d,%d,%d",myData->images[0]->iYear,
			myData->images[0]->iMonth,myData->images[0]->iDay,
			myData->images[0]->iHour,myData->images[0]->iMin,
			myData->images[0]->iuSec);
...
void InitializeTablesAndHandles(void)
{
	...
	********/
	...
	dataTableCtrl = NewCtrl (myApp.hExperiment, CTRL_TABLE_LS, "Table Control", 50, 900);
	myApp.dataTableCtrl = dataTableCtrl;
	InsertTableColumns (myApp.hExperiment,dataTableCtrl, -1, 1,VAL_CELL_STRING);		//Column for titles
...
{
	for(int i = 0; i<myApp.CExperimentDataCount;i++)
	{
		for(int j=0; j<20; j++){
		printf(myApp.CExperimentData_Array[i]->images[j]->TimeStamp);
		printf("\n");
		}
	}
...
int CVICALLBACK ShowAllTimestamps (int panel, int control, int event,
								   void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
...
{
	char buf[25];
	sprintf(buf,"%d//%d",myApp.CExperimentDataCount, sizeof(myApp.CExperimentData_Array));
	SetCtrlVal(myApp.hCalibrationEx,PANEL3_TEXTMSG_3,buf);
	return;	
}
...
{
	int chosen = 0;
	GetCtrlVal(myApp.hCalibrationEx,PANEL3_RADIOBUTTON, &chosen);
	if(chosen == 1){
		DisplayImageFile (myApp.hCalibrationEx, PANEL3_PICTURE_1, myApp.CExperimentData_Array[0]->images[0]->Image_PathName);	   		
		DisplayImageFile (myApp.hCalibrationEx, PANEL3_PICTURE_2, myApp.CExperimentData_Array[0]->images[1]->Image_PathName);
		DisplayImageFile (myApp.hCalibrationEx,	PANEL3_PICTURE_3, myApp.CExperimentData_Array[0]->images[2]->Image_PathName);
		DisplayImageFile (myApp.hCalibrationEx,	PANEL3_PICTURE_4, myApp.CExperimentData_Array[0]->images[3]->Image_PathName);	  
		DisplayImageFile (myApp.hCalibrationEx,	PANEL3_PICTURE_5, myApp.CExperimentData_Array[0]->images[4]->Image_PathName);	  
	}
	
	else if(chosen != 1){
		GetCtrlVal(myApp.hCalibrationEx,PANEL3_RADIOBUTTON_2, &chosen);
		if(chosen ==1)
		{
			DisplayImageFile (myApp.hCalibrationEx, PANEL3_PICTURE_1, myApp.CExperimentData_Array[1]->images[0]->Image_PathName);	   		
			DisplayImageFile (myApp.hCalibrationEx, PANEL3_PICTURE_2, myApp.CExperimentData_Array[1]->images[1]->Image_PathName);
			DisplayImageFile (myApp.hCalibrationEx,	PANEL3_PICTURE_3, myApp.CExperimentData_Array[1]->images[2]->Image_PathName);
			DisplayImageFile (myApp.hCalibrationEx,	PANEL3_PICTURE_4, myApp.CExperimentData_Array[1]->images[3]->Image_PathName);	  
			DisplayImageFile (myApp.hCalibrationEx,	PANEL3_PICTURE_5, myApp.CExperimentData_Array[1]->images[4]->Image_PathName);	  		
		}
	}
...
int CVICALLBACK DisplayCurrentCExperiment (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
		case EVENT_COMMIT:
			DisplayCurrentCExperiment1();
			break;
	}
	return 0;
}

int CVICALLBACK DisplayCurrentCExperiment2 (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
	...
	if( 1 ) {
		...
		********************************/
		...
		if(bFm){
			...
			// Fill buffer 'data' for generation of red actinic
			// GenSquareWaveRA(int numElementsAO, int numElementsAI,double scaledV, double scaledI, int bBA_ON, double squareWave[], double temp1[], double temp2[], double temp3[])
			GenSquareWaveRA (TotalsampsPerCycleAO,sampsPerChan, ampV, ampI, bBA_ON, dataAO,temp1,temp2,temp3);
		}
		...
		case EVENT_COMMIT:
			...
			if (control == PANEL_2_CB_F_FM_GenSeq){
				...
				*/  
				...
				/************************
				*
				* int bFirst, int bLast, int bFm, int bBA_ON
				*
				**********/
				bFm = 1;							// testing code with bFm first
				
				
				Gen_F_SequenceFn( rate,sampsPerCycle,0,0,bFm,bBA_ON);
