...
extern int CVIFUNC		cviprefix(ReadFile)(int handle, char *buf, size_t count);
extern int CVIFUNC		cviprefix(WriteFile)(int handle, const char *buf, size_t count);
...
#define WriteFile1   	cviprefix(WriteFile)
...
extern int CVIFUNC		ReadFile(int handle, char *buf, size_t count);
extern int CVIFUNC		WriteFile(int handle, const char *buf, size_t count);
extern int CVIFUNC		SetCommitMode(int commit);
#endif
#endif

extern int CVIFUNC		GetFileInfo(const char *filename, ssize_t *size);
extern ssize_t CVIFUNC	SetFilePtr(int handle, ssize_t offset, int origin);
extern int CVIFUNC		CompareBytes(const char* s1, int s1i, const char* s2, 
							int s2i, size_t n, int case_sens);
extern int CVIFUNC		CompareStrings(const char* s1, int s1i, const char* s2,
							int s2i, int case_sens);
extern void CVIFUNC		CopyBytes(char* d, int di, const char* s, int si, size_t n);
extern void CVIFUNC		CopyString(char* d, int di, const char* s, int si, ssize_t max_n);
extern int CVIFUNC		FindPattern(const char s[], int si, ssize_t n, const char* pattern,
							int case_sens, int start_from_right);
extern int CVIFUNC		ReadLine(int handle, char line[], ssize_t max_n);
...
extern int CVIFUNC		WriteLine(int handle, const char line[], ssize_t n);
extern void CVIFUNC		FillBytes (char *d, int di, size_t n, int ch);

extern int CVIFUNC		ArrayToFile (const char *filename, const void *array, int dataType,
							size_t numElements, size_t numGroups, int dataOrder,
...
#define VAL_INTPTR_T                    12L
#define VAL_UINTPTR_T                   13L

#define VAL_SIZE_T						VAL_UINTPTR_T
#define VAL_SSIZE_T						VAL_INTPTR_T
#define VAL_PTRDIFF_T					VAL_INTPTR_T
