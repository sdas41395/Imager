...
void PS_BP_On()
{
	...
	strncpy (strData, "PV 25\r", sizeof("PV 25\r"));
	stringsize = strlen (strData);
	ComWrt (3, strData, stringsize);
	DelayWithEventProcessing (.1);
...
{
	/********
	*	ToggleCntr (callback) calls ToggleCntrFn to toggle the counter value for the calling 
	*		toggle button.
	*
	*  TCBlueAct: Dev1/Ctr2 
	*/
...
int ToggleCntrFn (int control, int control_value) {
	...
	if(control == PANEL_4_BA_TCBA){
		...
		if (control_value) {
			double value = myApp.appCal.calBAControl.BA_Voltage;
			AO_BAVoltFn(value);
		...
		***************************************/
		...
		if (control_value) {
			double value_V = myApp.appCal.calBPControl.BP_Voltage;
			double value_I = myApp.appCal.calBPControl.BP_Current;
			AO_RAV_I_Fn(value_V,value_I);
		
		}
		
		/****************************************************************************************
		* 	Generate TC Blue Pulse signal  "Dev1/Ctr2" NO---> [- output "Dev1/port0/line3"] 
		*		TODO: REDO ---> ["Dev2/PFI3" ---> DIO3(PFI3/PO.3) ]
		*		taskHandleNOW?
		*
		***************************************/
...
int AO_BPV_I_Fn(double v_BP, double i_BP)
{
	...
	******/
	...
	TaskHandle thBP_V;  // taskhandle for BP (blue pulse) voltage
	double i_Test;
	double v_Test;
	double gain_vBP = 9;
	...
	if (v_Test > maxV)
	{
		sprintf(strTemp,"Voltage value is out of bounds for Blue Pulse (BA).%fx\n \n So nothing done\n", v_Test);
