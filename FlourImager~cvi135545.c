...
void SetBluePulseIntensity(double nCurrent, double nVoltage)
...
void TurnBPOn2(void)
{
		...
		TaskHandle* taskHandleBP; 
		TaskHandle* taskHandleForBPOn;
		taskHandleForBPOn = malloc(sizeof(TaskHandle));
		taskHandleBP = malloc(sizeof(TaskHandle));  
		*taskHandleBP = 0;
		*taskHandleForBPOn = 0; 
		int error=0;  
		uInt8 dataD[8] = {0};
		...
		myCalibration = myApp.appCal;
		int flag = 0;
		GetCtrlVal(myApp.hCalibration,PANEL1_RADIOBUTTON_3,&flag);
		
		//myCalibration.calBPControl.taskHandleForBPOn
...
int DisplayAndSaveImageData()
{
	...
	for(int i = 0; i<5; i++)	 //The loop will go through every picture in the 
