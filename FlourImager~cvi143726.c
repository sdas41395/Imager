...
int Cam_Picture()
{
	...
	***********************************************/
	...
	Image* image;
	DisplayMapping *mapping = malloc(sizeof(*mapping));
	/*++++++++++
	* 
	*  CVIDEF.h:
	...
	*	camera
	...
	// Create path for b16 images
	strcpy(fileName,local_pathname);		   
	
	// Create path for tif
	strcpy(tiff_filename,local_pathname);
	
	// make app information local
	// When data is generated it is then stored in the application struct myApp.appData: make local then send back
	CExperimentData myData;
	Bild pic;						 
	pic = myApp.appBild;

	unsigned short *pBuffer;  // holding "buffer" data from camera buffer put in after download
							  //   Camera buffer is myApp.data
	pBuffer = malloc(myApp.size);
	
	myApp.appData->images = /*(unsigned int)*/malloc((myApp.appCal.calRAControl.NumberOfPictures)*(sizeof(FI_Image)));
	if(myApp.appData->images == NULL)
	{
	...
	{
		myApp.appData->images[i] = malloc(sizeof(FI_Image)); // Mallocing every FI_Image in the array
		if(myApp.appData->images[i] == NULL)
		{
			MessagePopup("Malloc Error", "Error in mallocing the app data"); //Error check	
		}
		
		myApp.appData->images[i]->meta_image = malloc(myApp.size + 256); //mallocing the meta images in the fi images
		if(myApp.appData->images[i]->meta_image == NULL)
		{
			MessagePopup("Malloc Error", "Error in mallocing the app data");	//Error check
		}
	}
	
	// counters for unloading bcd information from raw data
	...
	*/ 
	...
	for (int i=1; i<=dwValidImageCnt; i++) 
	{
		...
		*/ 
		...
		unsigned char byte[112] = {'0'};    // what is purpose of this variable? 
										 // byte holds the raw data which we print 
		count_ = 1; // !!!!! index variable set to 1
		unsigned short test=0;
		
		for (int i=0; i<14; i++)		//TODO: FIX: i is set from 0 to [7]- no because an unsigned short has two bytes or 1 pixel !!!!
		...
		/*******
		* Filling textbox - add line for each picture
		*/
		for(int j=1; j<56; j++)
		{
		...
		iRetCode = imaqArrayToImage(image,pBuffer,myApp.wXResAct, myApp.wYResAct);   //taking the image from FI_Image
		if (iRetCode == ERR_SUCCESS) {
			error_code = imaqGetLastError();
			strcpy(errBuff, imaqGetErrorText(error_code));
		...
		if (i <= 5)
		{
			...
			mapping->method = IMAQ_90_PCT_DYNAMIC_ALWAYS;
			...
			*/
			...
			if (iRetCode == ERR_SUCCESS ) {
				...
				//sprintf(strTemp,"Image not saved beacuse of file error. Probably an access rights problem.%lx\n", iRetCode);
				MessagePopup("IMAQ error",strTemp);
			}
		}
		...
		*/
		...
		//Checking counter of picture to confirm
		
		int Picture_Check = 0;
		Picture_Check = CounterCheck(byte, row_counter);  //row_counter starts at 1
		if (Picture_Check == 0)		//if equal to 1 then it was a success
		{
			MessagePopup("Error in obtaining Picture", "Correct counter was not detected for picture// Pictures not saved");
			return 0;	
		}
		//MessagePopup("Successful Counter", "Counter is on point");
		
		
		/******************************Check done*/
	
		FI_Image* tempFI;							//Creation of a temporary holder FI_image that will contain pBuffer and
		tempFI = malloc(sizeof(FI_Image));		//be put into our array of FI_Images// Malloced with the size of the struct
		tempFI->meta_image = malloc(myApp.size);  //Malloced normally(The size of pBuffer or myApp.size
		
		if(tempFI == NULL || tempFI->meta_image == NULL)					   //Simple Error checking to see if the malloc
		{																   //was successfull or simply malloced nothing
			MessagePopup("Malloc","there has been in error in intializingasdfasd the temp FI_Image in campic");	
		}
		
		
		tempFI->meta_image = pBuffer; 			 //the temp FI_image is saving pBuffer to its meta_image. pBuffer is discarded
	
		
		//****** Finished Filling Image Array with images ***********
		
		
		
		//****** Saving TimeStamp to our FI_IMAGE *******
	
		//The 14 bits of the timestamp will be saved to the temp->TimeStamp before being transferred completely to
		//myApp.appData->images[row-counter - 1] which is holding all of these temp FI_Images. All malloced so its okay 
		
		memcpy(tempFI->TimeStamp,byte,56); //byte like timestamp is an unsigned char[100] that contains first 14bytes. 
								//we can individually access each byte by running it through a loop and calling it
								//byte[i]; 
		
		int tempCnt = (int)(byte[13])*10 + (int)(byte[14]);
		char mess[50] = "asdasd";
		sprintf(mess, "Counter is: %d /n Row count is: %d", tempCnt,row_counter);
		MessagePopup("Saving Timestamp", mess);
								
		/*						
		tempFI->iYear  = ((int) byte[17] *1000) + ((int) byte[18] * 100) + ((int) byte[19] *10) + ((int) byte[20]);
		tempFI->iPicCnt = row_counter;
		tempFI->iMonth = (int) byte[7];
		if(tempFI->iMonth <= 0 || tempFI->iMonth > 12)
		{
			char err_mess[50] = "asdas";
			sprintf(err_mess,"incorrect day: %d", tempFI->iMonth);
			MessagePopup("Error in Saving Timestamp", err_mess);	
		}
		tempFI->iDay = *(signed char *)(&byte[8]);
		
		if(tempFI->iDay <=0 || tempFI->iDay > 31)
		{
			char mess[50] = "asdasd";
			sprintf(mess, "Incorrect month: %d", tempFI->iDay);
			MessagePopup("Error in Saving Timestamp", mess);	
		}
		tempFI->iHour = *(signed char *)(&byte[9]);
		if(tempFI->iHour < 0 || tempFI->iHour>23)
		{
			MessagePopup("Error in Saving Timestamp", "Hour is incorrect");	
		}
		tempFI->iMin = *(signed char *)(&byte[10]);
		if(tempFI->iMin < 0 || tempFI->iMin > 59)
		{
			MessagePopup("Error in Saving Timestamp", "Min is incorrect");	
		}
		tempFI->iSec = *(signed char *)(&byte[11]);
		if(tempFI->iSec <0 || tempFI->iuSec > 59)
		{
			MessagePopup("Error in Saving Timestamp", "Seconds is incorrect");	
		}
		tempFI->iuSec = *(signed char *)(&byte[12])*10000 + *(signed char *)(&byte[13])* 100 + 
						*(signed char *)(&byte[14]);
		
								
		*/						
		//*********	Finished saving TimeStamp ****
			
		
		/* Saving data into the Data Array for Application struct ***/
		
		 myApp.appData->images[row_counter- 1] = tempFI;		//saving the FI_Image into our array of FI_Images  
		 													//row_counter is subtracted by 1 because it starts with 1 and we
															//start with 0 for arrays
		row_counter ++;		//row_counter is the picture number being unloaded from the camer													
		/******* Finished Pushing *****/
		//At this point the myApp.appData has the timestamp, pathname, and complete unsigned short for all 
		//images. It will be processed outside of the collection loop in different functions 
			
		}
		/**************** Finished looping through download of images in camera and saving them.******/
		
		
		
		//This is outside the whole camp picture loop and where it will be displaying and Saving the images.
		
		// DisplayAndSaveImageData();	   //This function will run through the amount of images and display them 
							   // using nimaq functions. CVI functionality has not been reached yet.
		
		
		/******* Creating A Histogram to be saved into struct**************************************/
		
		//This is called inside of Display and Save Image Data since it relies on the
		//image variable
		
		
		
		/******* Finished Creating and Saving Histogram********************************************/
...
// End of image processing for each image read in so there is a buffer of raw data
	
	// Put local data back. Disabled temporarily for Testing. These local variables must be created
							//with every single function. much more efficient to just make it a global and
							//then pass back into myApp. For now i reverted it back to a global to avoid
							//dealing with uninitialized local variables for all functions. 
...
void InitializeTablesAndHandles(void)
{
	...
	****/
	...
	//***********Filling the array of images**************/
	
	/*unsigned short *images[5] = {
		myData.image1,
		myData.image2,
		myData.image3,
		myData.image4,
...
int DisplayAndSaveImageData()
{
	...
	//This function will take the (?) unsigned image data in our array and turn them into images while saving them onto
	//the pathname specified in the main app and initialized from the Calibration panel
	
	int		iRetCode = 0;             	//This is an error check. If <0 then a problem has occurred
	int		error=0;
	char	errBuff[2048]={'\0'}; 
	char 	strTemp [100] = " ";		//debug string use
	...
	Image* image;					//The image that will be created using the myApp.appData->images[i]->meta_image data
	int numCols = myApp.wXResAct;   //Setting the size boundaries of the image
	int numRows = myApp.wYResAct;
	
	float min = -5;	     		//Min number the pixel can be assigned to in histogram						  
	float max = 1050;	 		//max number the pixel can be assigned to in histogramint numClasses = 480000;	//Number of total pixels in the picture to be included in histogram
	Image*	mask = NULL; 		//mask Image to get different results. Not sure if useful set to NULL
	int numClasses = 480000;
	...
	for(int i = 0; i<myApp.appCal.calRAControl.NumberOfPictures; i++)	 //The loop will go through every picture in the 
	{																	 //Number of pictures on the table 

	
		sprintf(PictureNameb16,"Pictures_b16\\test%d.b16",i);				  //These two lines will be adding test plus the number of the image
		MakePathname(local_pathname, PictureNameb16, file_b16);    
		
		sprintf(PictureNametif,"Pictures_tif\\test%d.tif",i);               
		MakePathname(tiff_filename, PictureNametif,file_tif);  
		strcpy(myApp.appData->images[i]->Image_PathName, tiff_filename);
		//MessagePopup("Display and Saving tif file name",file_tif);
		...
		iRetCode = imaqSetBitDepth(image, 14);				  //Does this work with tiff files
		iRetCode = imaqSetImageSize(image,numCols,numRows);
		iRetCode = imaqArrayToImage(image,myApp.appData->images[i]->meta_image,numCols,numRows);   //taking the image from FI_Image
		
		if (5<< i <= 10)
		{
			iRetCode = imaqDisplayImage(image,i, TRUE);
			iRetCode = imaqSetWindowDisplayMapping( i, IMAQ_90_PCT_DYNAMIC_ALWAYS );//This displays it on only one window (0) so it will constantly be updated
		}												//alternatively we can create different windows to open it on 
														//by replacing 0 with i
		...
		// Setting aray of pic struct to pBuffer.  Setting the array for the camware
		//    routine ---- store_b16
		pic.pic12 = myApp.appData->images[i]->meta_image;
		// Sending meta_image twice?
		iRetCode = store_b16(file_b16, myApp.wXResAct, myApp.wYResAct, myApp.appData->images[i]->meta_image, (Bild*)&pic.pic12); 
  		if (iRetCode != PCO_NOERROR ) {
			sprintf(strTemp,"Image not saved beacuse of file error. Probably an access rights problem.%lx\n", iRetCode);
			MessagePopup("Camera Message",strTemp);
		}
		
		//This must be done in this loop as it relies on the image variable
		// *****This does not work
		// histogram(image,min,max,mask,numClasses); //Will create histogram for every FI_Image now 
	
	}
		 //This will be reusing the image variable used earlier : image = imaqCreateImage();
Error:
...
int CVICALLBACK Display_All (int panel, int control, int event,
							   void *callbackData, int eventData1, int eventData2)
{
	if(event == EVENT_COMMIT)
	{
...
void image_display_func()
{
	...
	{
		int count = 7;
		DisplayImageFile (myApp.hCalibrationEx, count, myApp.appData->images[i]->Image_PathName); 
		count = count +1;
	}
...
int BACalibrationFn ()
{
	...
	********************/
	...
	iNumPts = (int)((myApp.appCal.calBAControl.BA_V_HB - myApp.appCal.calBAControl.BA_V_LB)/increment)+10;
	// size arrays accordingly
	if( (dataAcq=malloc(sampsPerCycle*sizeof(float64)))==NULL ) {
		MessagePopup("Error","Not enough memory");
		goto Error;
	}
	if( (dataX_volts=malloc(iNumPts*sizeof(double)))==NULL ) {
		MessagePopup("Error","Not enough memory");
		goto Error;
	}
	...
	int count = 0;
	for (v=myApp.appCal.calBAControl.BA_V_LB;v<myApp.appCal.calBAControl.BA_V_HB;v=v+increment) {
		voltBA = v;
		AO_BAVoltFn( voltBA);
	...
	****/
		...
		// Set up XAxis
		// set graph controls for time base
		SetCtrlAttribute(myApp.hCalibration,PANEL1_GRAPH,ATTR_XAXIS_GAIN,1/((double)iNumPts));
		log = (int)log10(iNumPts);
		SetCtrlAttribute(myApp.hCalibration,PANEL1_GRAPH,ATTR_XPRECISION,log);
