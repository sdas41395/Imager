...
//==============================================================================
// Static functions  - only used in this c file?

// Generation of serial data to output waveforms to AO
...
static void GenSquareWaveRA(int numElementsAO, int numElementsAI, double scaledV, double scaledI, double baVolt, int bBA_ON, double squareWave[], double temp1[], double temp2[], double temp3[]);

static void GenPS_Sequence(int numElements, double amplitude, int timeBaseNumber, double sequence[], double temp1[], double temp2[], double temp3[], int bBA_ON);
...
int 	Gen_F_SequenceFn (float64 rate, uInt32 sampsPerCycle, int bFirst, int bLast, int bFm, int bBA_ON);
...
int main (int argc, char *argv[])   
{
	...
	*/
	...
	//Initialize and ? (TODO) the U IR functions
...
static void GenSquareWave(int numElements, double amplitude, double frequency, double squareWave[], double temp1[], double temp2[], double temp3[])
{
	/*****************************************************
	* GenSquareWave
	* 	Date: original taken from NI sample code
	* 	Authour: SWR
	*	Notes:
	*			int numElements, 
	*			double amplitude, 
...
static void GenSquareWaveRA(int numElementsAO, int numElementsAI,double scaledV, double scaledI, double baVolt, int bBA_ON, double squareWave[], double temp1[], double temp2[], double temp3[])
{
	...
	***********************************/
	...
	if (!bBA_ON){
		...
		pointGain = numElementsAI*3/ numElementsAO;
	}
...
static void GenPS_Sequence(int numElementsAO, double amplitude, int timeBaseNumber, double sequence[], double temp1[], double temp2[], double temp3[], int bBA_ON)
{
	...
	* 	Authour: SWR
	*	Notes:
	*		Derived from GenSquareWave.... Changed order of Blue Actinic so that it could be turned on and off
	*		(GenSquareWaveRA(int numElementsAO, int numElementsAI,double scaledV, double scaledI, int bBA_ON, double squareWave[], double temp1[], double temp2[], double temp3[]))
	*
	*******************************************************/
	...
	if (!bBA_ON){
		...
		pointGain = timeBaseNumber*3/ numElementsAO;
	}
...
int Cam_Picture()
{
	...
	***********************************************/
	...
	char	strTemp[100];  		//str used for  for 
	char 	strTemp1[100]; 
	...
	* Struct declaration
	*********/
	// Imaq variables and struct DisplayMapping for the Image struct
	Image* image;
	DisplayMapping *mapping = (DisplayMapping *)malloc(sizeof(*mapping));
	
	// Creating local pointer to a CExperiment data structure on heap
	CExperimentData * myData;
	myData = (CExperimentData *)malloc(sizeof(CExperimentData));
	if(myData == NULL) {
		MessagePopup("Error mallocing","Not enough memory for CExperiment Struct in Cam_Picture");
		goto Error;
	} 
	...
	*	camera
	...
	/**********
	*  Mallocing buffer for image data downloaded
	***********/
	...
	if((pBuffer)==NULL) {
		MessagePopup("Error in Cam_Picture","Not enough memory for pBuffer");
		goto Error;
	} 
	...
	*/
	...
	if (iRetCode != PCO_NOERROR)
	{
		...
		MessagePopup("Camera Error in Cam_Picture",strTemp);
	}
	...
	**********/
	...
	myApp.CExperimentData_Array[myApp.CExperimentDataCount].imageCount = dwValidImageCnt; 

	
	// DEBUG/ERRORCHK
	//  Verify camera has all the images it was supposed to take
	// TODO Too many problems with counters to use this now
	/*
	...
	*/
	//for (int k=0; k <dwValidImageCnt;k++){
...
//	}
	...
	*/
	...
	for (int i=1; i<=dwValidImageCnt; i++) 
	{
		...
		myApp.CExperimentData_Array[myApp.CExperimentDataCount].images[i-1].meta_image = (unsigned short *)malloc(myApp.size + 256);
		...
		*/
		...
		for (int k = 0; k < myApp.wXResAct*myApp.wYResAct; k++){
			...
			myApp.CExperimentData_Array[myApp.CExperimentDataCount].images[i-1].meta_image[k] = myApp.data[k];
		}
		...
		*/ 
		...
		for (int i=0; i<14; i++)			//need 14 pixels or bytes to determine the date and time stamp
		{							
		...
		*/
		// myData use...
		myData->images[i-1].TimeStamp[0] = (int)((int)byte[13]*10 + (int)byte[14]);
		myData->images[i-1].TimeStamp[1] = (int)((int)byte[17]*1000 + (int)byte[18]* 100 + (int)byte[21]*10 + (int)byte[22]);  
		myData->images[i-1].TimeStamp[2] = (int)((int)byte[25]*10 + (int)byte[26]);  
		myData->images[i-1].TimeStamp[3] = (int)((int)byte[29]*10 + (int)byte[30]);  
		myData->images[i-1].TimeStamp[4] = (int)((int)byte[33]*10 + (int)byte[34]);  
		myData->images[i-1].TimeStamp[5] = (int)((int)byte[37]*10 + (int)byte[38]);  
		myData->images[i-1].TimeStamp[6] = (int)((int)byte[41]*10 + (int)byte[42]);
		
		
		// for(int i =1; i<=; i++){
		myApp.CExperimentData_Array[myApp.CExperimentDataCount].images[i-1].TimeStamp[0] = myData->images[i-1].TimeStamp[0];
		myApp.CExperimentData_Array[myApp.CExperimentDataCount].images[i-1].TimeStamp[1] = myData->images[i-1].TimeStamp[1];
		myApp.CExperimentData_Array[myApp.CExperimentDataCount].images[i-1].TimeStamp[2] = myData->images[i-1].TimeStamp[2];
		myApp.CExperimentData_Array[myApp.CExperimentDataCount].images[i-1].TimeStamp[3] = myData->images[i-1].TimeStamp[3];
		myApp.CExperimentData_Array[myApp.CExperimentDataCount].images[i-1].TimeStamp[4] = myData->images[i-1].TimeStamp[4];
		myApp.CExperimentData_Array[myApp.CExperimentDataCount].images[i-1].TimeStamp[5] = myData->images[i-1].TimeStamp[5];
		myApp.CExperimentData_Array[myApp.CExperimentDataCount].images[i-1].TimeStamp[6] = myData->images[i-1].TimeStamp[6];
		// }
	...
	// old struct manipulations
	//myApp.CExperimentData_Array[myApp.CExperimentDataCount] =  *myData;
	//myApp.CExperimentData_Array[myApp.CExperimentDataCount]->data_table_count = myApp.CExperimentDataCount;
	
	
	/**************************
	*  Saving data images into table from array of CExperimentData structs (
	*
	*********/ 
	// insert one row for this newly created CExperimentData struct which was placed into the array
	InsertTableRows(myApp.hExperiment,myApp.dataTableCtrl,-1,1,VAL_USE_MASTER_CELL_TYPE);
	
	//Fill title cell for data struct
	char name[100];
	sprintf(name,"No. Pics =  %d, (%d)",myApp.dwValidImageCnt, myApp.CExperimentDataCount);
	SetTableCellVal (myApp.hExperiment,myApp.dataTableCtrl, MakePoint(1,myApp.CExperimentDataCount+1),name);
	
	//insert Display button at cell 3
	char display[20];
	sprintf(display,"Display");
	
	SetTableCellAttribute (myApp.hExperiment, myApp.dataTableCtrl,
                       MakePoint(3,myApp.CExperimentDataCount+1), ATTR_CELL_TYPE,
                       VAL_CELL_BUTTON); 
	SetTableCellVal(myApp.hExperiment,myApp.dataTableCtrl,MakePoint(3,myApp.CExperimentDataCount+1),display);
	
	// insert delete button at cell 4
	char delete[20];
	sprintf(delete,"Delete");
	SetTableCellAttribute (myApp.hExperiment, myApp.dataTableCtrl,MakePoint(4,myApp.CExperimentDataCount+1), 
						   ATTR_CELL_TYPE, VAL_CELL_BUTTON);
	SetTableCellVal(myApp.hExperiment,myApp.dataTableCtrl,MakePoint(4,myApp.CExperimentDataCount+1),delete);
	
	// insert save button at cell 5
	sprintf(strTemp,"Save");
	SetTableCellAttribute(myApp.hExperiment, myApp.dataTableCtrl,MakePoint(5,myApp.CExperimentDataCount +1),
						  ATTR_CELL_TYPE,VAL_CELL_BUTTON);
	SetTableCellVal(myApp.hExperiment,myApp.dataTableCtrl,MakePoint(5,myApp.CExperimentDataCount+1),strTemp);

	//Connection to the callback.
	InstallCtrlCallback(myApp.hExperiment, myApp.dataTableCtrl, Data_Table, 0);
	
	//Timestamp ... something ????????????????????????????????????
	// ********* should be from CExperiment struct in array not local myData. Assigning timestamp from my Data to CExperiment Array
	
	char TimeStamp[20];
	sprintf(TimeStamp, "%d,%d,%d\\%d,%d,%d",myApp.CExperimentData_Array[myApp.CExperimentDataCount].images[0].TimeStamp[1],
			myApp.CExperimentData_Array[myApp.CExperimentDataCount].images[0].TimeStamp[2],myApp.CExperimentData_Array[myApp.CExperimentDataCount].images[0].TimeStamp[3],
			myApp.CExperimentData_Array[myApp.CExperimentDataCount].images[0].TimeStamp[4],myApp.CExperimentData_Array[myApp.CExperimentDataCount].images[0].TimeStamp[5],
			myApp.CExperimentData_Array[myApp.CExperimentDataCount].images[0].TimeStamp[6]);
	SetTableCellVal(myApp.hExperiment,myApp.dataTableCtrl,MakePoint(2,myApp.CExperimentDataCount+1),TimeStamp);
	
	//******Making the table fit to text
	for(int i=1; i<5; i++)
	{
		SetColumnWidthToWidestCellContents(myApp.hExperiment, myApp.dataTableCtrl,i);
	}
	SetRowHeightToTallestCellContents(myApp.hExperiment, myApp.dataTableCtrl,myApp.CExperimentDataCount +1);  
	SetTableCellAttribute (myApp.hExperiment, myApp.dataTableCtrl,
                       MakePoint(1,myApp.CExperimentDataCount+1), ATTR_WRAP_MODE, VAL_CHAR_WRAP);
	SetTableCellAttribute (myApp.hExperiment, myApp.dataTableCtrl,
                       MakePoint(2,myApp.CExperimentDataCount+1), ATTR_WRAP_MODE, VAL_CHAR_WRAP);
	SetTableCellAttribute (myApp.hExperiment, myApp.dataTableCtrl,
                       MakePoint(3,myApp.CExperimentDataCount+1), ATTR_WRAP_MODE, VAL_CHAR_WRAP);
	SetTableCellAttribute (myApp.hExperiment, myApp.dataTableCtrl,
                       MakePoint(4,myApp.CExperimentDataCount+1), ATTR_WRAP_MODE, VAL_CHAR_WRAP);
	for(int i=1; i<5; i++){
		SetColumnWidthToWidestCellContents (myApp.hExperiment,
                                    myApp.dataTableCtrl, i);
		SetRowHeightToTallestCellContents (myApp.hExperiment,
                                    myApp.dataTableCtrl, myApp.CExperimentDataCount+1);
	}
	//******End formatting row and col sizes
	
	// CExperiment Data Count is the position in the array of data structs
	myApp.CExperimentDataCount++;  //  = CExperimentDataCount++;
...
Error:
	...
	//Cant free myData. Every iteration of myData will be pointed at by some point in the CExperimentData array
	//free(myData);    
	*/
	return 0;	
...
{
	/*****************************************
	* Title:  Data_Table (CALLBACK)
	* Author: 	Shoham Das and Stuart Rose
	* Date: 	4/17/2016
	* Note: Displays the pictures when display button is pressed on certain sequence ro	
	*
	***********************************************/
	...
	int 	i; 						// iterator for image data array
	...
	if( event==EVENT_COMMIT){
		...
		tempData = myApp.CExperimentData_Array[eventData1 - 1];
		image =   imaqCreateImage(IMAQ_IMAGE_U16, 0);
			if(eventData2 == 3 && eventData1 != 0){
			//this handler process button push on third column, the display button
			// display all valid images or 5 images	
				for(i=0; i<5 && i<tempData.imageCount; i++){
					...
					iRetCode = imaqSetBitDepth(image, 14);				  
					...
					iRetCode = imaqArrayToImage(image,tempData.images[i].meta_image,myApp.wXResAct, myApp.wYResAct);   //taking the image from FI_Image
					if (iRetCode == ERR_SUCCESS) {
						error_code = imaqGetLastError();
						strcpy(errBuff, imaqGetErrorText(error_code));
						sprintf(strTemp,"imaq Error code %lx\n And Returned error code:  %lx\nAnd text out: %c", error_code, iRetCode,*errBuff);
						MessagePopup("Imaq During set during imaqArrayToImage",strTemp);
					}
					
					// Set title for displayed image with Image number; Time: Hour:Min:Sec
					sprintf(strTemp,"Image Number: %d ; Time: %d:%d:%d ", tempData.images[i].TimeStamp[0],tempData.images[i].TimeStamp[4],
						   tempData.images[i].TimeStamp[5],tempData.images[i].TimeStamp[6]); 
					iRetCode = imaqSetWindowTitle(i, strTemp);
					if (iRetCode == ERR_SUCCESS) {
						error_code = imaqGetLastError();
						strcpy(errBuff, imaqGetErrorText(error_code));
						sprintf(strTemp,"imaq Error code %lx\n And Returned error code:  %lx\nAnd text out: %c", error_code, iRetCode,*errBuff);
					...
					mapping->maximumValue = 512;
		
			
					iRetCode = imaqSetWindowDisplayMapping( i, mapping );//This displays it on only one window (0) so it will constantly be updated
					if (iRetCode == ERR_SUCCESS) {
						error_code = imaqGetLastError();
						strcpy(errBuff, imaqGetErrorText(error_code));
						sprintf(strTemp,"imaq Error code %lx\n And Returned error code:  %lx\nAnd text out: %c", error_code, iRetCode,*errBuff);
					...
					iRetCode = imaqDisplayImage(image,i, TRUE);
					if (iRetCode == ERR_SUCCESS) {
						error_code = imaqGetLastError();
						strcpy(errBuff, imaqGetErrorText(error_code));
						sprintf(strTemp,"imaq Error code %lx\n And Returned error code:  %lx\nAnd text out: %c", error_code, iRetCode,*errBuff);
			...
			if(eventData1 != 0 && eventData2 == 4){
				//This deletes the data at the fourth button which is delete. 
				for(int i2 = 0; i2<myApp.CExperimentData_Array[eventData1].imageCount;i2++){
					free(myApp.CExperimentData_Array[eventData1].images[i2].meta_image);
				}
				for ( int c = eventData1 - 1;c < myApp.CExperimentDataCount-1;c++){
					myApp.CExperimentData_Array[c] = myApp.CExperimentData_Array[c+1];
				}
				DeleteTableRows(myApp.hExperiment,myApp.dataTableCtrl,eventData1, 1);
			}
			
			if(eventData1 != 0 && eventData2 == 5){
				// Event Processing for Save button (column 5)
				for(i=0; i<myApp.CExperimentData_Array[eventData1].imageCount; i++){
					// Create imaq image struct
					image =   imaqCreateImage(IMAQ_IMAGE_U16, 0);
					if (image == NULL) {
						error_code = imaqGetLastError();
						strcpy(errBuff, imaqGetErrorText(error_code));
					...
					// set image stuct to depth of bits collected by camera - should use pco Bild struct
					...
					// set image struct to size in app struct
					iRetCode = imaqSetImageSize(image,myApp.wXResAct, myApp.wYResAct);
					// move array from tempData (local copy of stuct)
					iRetCode = imaqArrayToImage(image,myApp.CExperimentData_Array[eventData1].images[i].meta_image,myApp.wXResAct, myApp.wYResAct);   //taking the image from FI_Image
					if (iRetCode == ERR_SUCCESS) {
						error_code = imaqGetLastError();
						strcpy(errBuff, imaqGetErrorText(error_code));
							...
							void *callbackData, int eventData1, int eventData2)
{
	...
	if( event==EVENT_COMMIT ) {
		...
		*/
		// Controls for getting AI time base info 
		GetCtrlVal(myApp.hCalibration,PANEL1_SAMPSPERCHAN,&sampsPerChannel);
		GetCtrlVal(myApp.hCalibration,PANEL1_RATE,&rate); 
		...
		****/
		...
		if(bBA_ON){
			baVolt = myApp.appCal.calBAControl.BA_Voltage;
			// Fill buffer 'data' for generation of blue actinic and red actinic
			GenSquareWaveRA (TotalSampsAO, sampsPerChannel,ampV, ampI, baVolt, bBA_ON, dataAO,temp1,temp2,temp3);
		}
		else{
			// Fill buffer 'data' for generation of red actinic
			GenSquareWaveRA (TotalSampsAO,sampsPerChannel, ampV, ampI, baVolt, bBA_ON, dataAO,temp1,temp2,temp3);
							...
							void *callbackData, int eventData1, int eventData2)
{
	...
	if( event==EVENT_COMMIT ) {
		...
		*******************************************************/
		...
		ProcessDrawEvents();
		DelayWithEventProcessing (1.1); //in seconds  
	...
	`	*	VAL_RIGHT_YAXIS	The y-axis on the right side of the graph or strip chart.
		...
		****/
		...
		// Scale data voltage values to PAR sensor so output in uE
		for(int y=0; y<(numChannels * numRead); y++)
		{
			dataAcq[y] = dataAcq[y] * myApp.PARConst;
		}
		...
		*/
		...
		SetAxisScalingMode (myApp.hCalibration,PANEL1_GRAPH3, VAL_LEFT_YAXIS, VAL_AUTOSCALE, 0, 5.0);
		...
		if( numRead>0 )
			for(int j=0;j<numChannels;j++)
			{
				...
				PlotY(myApp.hCalibration,PANEL1_GRAPH3,&(dataAcq[j*numRead]),numRead,VAL_DOUBLE,VAL_THIN_LINE,VAL_EMPTY_SQUARE,VAL_SOLID,
					  1,plotColors[j%12]);
			}
...
void InitializeTablesAndHandles(void)
{
	/***********************
	* InitializeTablesAndHandles
	*	Notes: Initialize varialbes from table values
	*			Program UIR elements such as
	*				-On experiment panel initialize tabs
	*				-Table for holding data structs
	*
	*
	************************/
	...
	******/
	...
	// initialize a singular instance of CExperimentData
	CExperimentData *myData;
	...
	myApp.CExperimentDataCount = 0;                   
	
	/*******
	*  malloc array of CExperimentData 30 data structs
	*****/
	myApp.CExperimentData_Array = (CExperimentData *)malloc(30 * sizeof(CExperimentData));
	/*for(int i=0; i<5; i++){
	...
	/*****end setting up array to hold CExperimentData structs***/
	...
	*/
	// Power initialization does not belong here and control and values unknown
	/*
	PS_BP_Off();
	PS_RA_Off();
	...
	******/ 
	...
	/*** End taking information out of tables *****/
	...
	*****/
	...
	/*****************
	* Program controls on Experiment panel Displaying images on tabs on experiment panel: not working on locals 
	...
	****/
	...
	/******************************************************
	*
	* Place table under tabs with CExperiment structs
	********/
	
	int dataTableCtrl;
	
	dataTableCtrl = NewCtrl (myApp.hExperiment, CTRL_TABLE_LS, "Table Control", 650, 98);
	// Save id for dataTable control into app data structure
	myApp.dataTableCtrl = dataTableCtrl;
	InsertTableColumns (myApp.hExperiment,dataTableCtrl, -1, 1,VAL_CELL_NUMERIC);	//Column for titles
	InsertTableColumns (myApp.hExperiment,dataTableCtrl,-1,1,VAL_CELL_NUMERIC);		//Column for time stamp
	InsertTableColumns  (myApp.hExperiment,dataTableCtrl,-1,1,VAL_CELL_BUTTON);		//Column for display button
	InsertTableColumns  (myApp.hExperiment,dataTableCtrl,-1,1,VAL_CELL_BUTTON);		//Columns for delete button
	InsertTableColumns  (myApp.hExperiment,dataTableCtrl,-1,1,VAL_CELL_BUTTON);		//Columns for save button
	SetCtrlAttribute (myApp.hExperiment,dataTableCtrl,ATTR_COLUMN_LABELS_VISIBLE,1);
	
	// Set column header labels
	// column 1: title
	SetTableColumnAttribute (myApp.hExperiment,
							  dataTableCtrl,1,
							  ATTR_USE_LABEL_TEXT, 1);
	SetTableColumnAttribute (myApp.hExperiment,
							 dataTableCtrl,1,
							 ATTR_LABEL_TEXT, "Title");
	// column 2: time stamp
	SetTableColumnAttribute (myApp.hExperiment, dataTableCtrl, 2,ATTR_USE_LABEL_TEXT, 1);
	SetTableColumnAttribute (myApp.hExperiment,dataTableCtrl,2,ATTR_LABEL_TEXT, "Time stamp");
	
	// column 3: Display Button
	SetTableColumnAttribute (myApp.hExperiment,dataTableCtrl,3,ATTR_USE_LABEL_TEXT, 1);
	SetTableColumnAttribute (myApp.hExperiment,dataTableCtrl,3,ATTR_LABEL_TEXT, "Display Button");
	
	// column 4: Delete Button
	SetTableColumnAttribute (myApp.hExperiment, dataTableCtrl,4,ATTR_USE_LABEL_TEXT, 1);
	SetTableColumnAttribute (myApp.hExperiment,dataTableCtrl,4,ATTR_LABEL_TEXT, "Display Button");
	// column 5: Display Button
	SetTableColumnAttribute (myApp.hExperiment,dataTableCtrl,5,ATTR_USE_LABEL_TEXT, 1);
	SetTableColumnAttribute (myApp.hExperiment,dataTableCtrl,5,ATTR_LABEL_TEXT, "Display Button");
	
	
	// Make space for table TODO should make space flexible
	SetCtrlAttribute (myApp.hExperiment, dataTableCtrl,ATTR_WIDTH,600);
	SetCtrlAttribute (myApp.hExperiment, dataTableCtrl,ATTR_HEIGHT,400); 
								 ...
								 void *callbackData, int eventData1, int eventData2)
{
	...
	if( event==EVENT_COMMIT ) {
		...
		/************************************
		* Control values necessary for plotting output of this function TurnOnActinic
		*    TODO Control values should be collected during initialization and have call backs
		*****/
		// Controls for getting AI time base info 
		GetCtrlVal(myApp.hCalibration,PANEL1_SAMPSPERCHAN,&sampsPerCycle);
		GetCtrlVal(myApp.hCalibration,PANEL1_RATE,&rate);
		
		// load V and I values for RA which have been loaded into RAScVolt1 and RAScCurr1 from value 
		//		in local appCal copy - myCalibration (NOT scaled? so TODO: eliminate "Sc")
		//		probably is scaled (0-5 volts)
		ampV = RedActScVolt1;
		ampI = RedActScCurr1;
		...
		***********************************/
		...
		SetWaitCursor(1);
		//  Toggle/set value of PulseEnLow which is 0 
		PEL_ONOFF = 0;
		...
		****/
		...
		DAQmxErrChk (DAQmxSetAODACRefSrc(gTaskHandle1,chan,DAQmx_Val_External));
		DAQmxErrChk (DAQmxSetAODACRefExtSrc(gTaskHandle1,chan,"EXTREF"));
		// numChannels = 2  or 3 because this is RA voltage and current AO and possibly BA)
		DAQmxErrChk (DAQmxGetTaskAttribute(gTaskHandle1,DAQmx_Task_NumChans,&numChannelsAO));
		...
		sampsPerCycleAO = 1000; 
		TotalsampsPerCycleAO = numChannelsAO * sampsPerCycleAO;
		// This SampClkTiming configuration does not care about numChannelsAO
		DAQmxErrChk (DAQmxCfgSampClkTiming (gTaskHandle1, "", rateAO, DAQmx_Val_Rising, DAQmx_Val_FiniteSamps, sampsPerCycleAO));
		DAQmxErrChk (DAQmxRegisterDoneEvent(gTaskHandle1,0,DoneCallback,NULL));

		//Set up buffer Prepare for plotting 6733 Red Act "Dev2/ao5:6" and, possible Dev2/ao2
		//	 BA_PS_Voltage:	'Dev2/ao2'
		//   RA_PS_Voltage: 'Dev2/ao5'
		//   RA_PS_Current: 'Dev2/ao6'
		if( (dataAO=malloc(TotalsampsPerCycleAO*sizeof(float64)))==NULL ) {
			MessagePopup("Error","Not enough memory");
		...
		// AI / graphed time base: value taken from control on UI (above) so it is the number of samples 
		//  		needed given the sampsPerCycle of AI (number of samples in 1 sec)
		TotalsampsPerCycle = sampsPerCycle;
		...
		// TODO: eliminate this redundancy once GenPS_Sequence is working and being graphed correctly
		if(bBA_ON){
			// Fill buffer 'data' for generation of blue actinic and red actinic
			// GenSquareWave (TotalsampsPerCycle, 0, 1, dataAO,temp1,temp2,temp3);
			GenPS_Sequence(TotalsampsPerCycleAO, 0, TotalsampsPerCycle, dataAO,temp1,temp2,temp3,bBA_ON);
		...
		*/
		...
		// DAQmxErrChk (DAQmxLoadTask ("TCBluePulse",&taskHandle1);
		DAQmxErrChk (DAQmxCreateTask("",&taskHandle1));
		DAQmxErrChk (DAQmxCreateCOPulseChanTime(taskHandle1,"Dev1/Ctr1","",DAQmx_Val_Seconds,
												DAQmx_Val_Low,
												BPinitDelay,
												BPlowTime,
												BPhighTime));
		...
		*/ 
		...
		DAQmxErrChk (DAQmxCreateTask("",&taskHandle3));
		DAQmxErrChk (DAQmxCreateCOPulseChanTime(taskHandle3,"Dev1/Ctr3","",DAQmx_Val_Seconds,
												DAQmx_Val_Low,
												CamInitDelay,
												CamLowTime,
												CamHighTime));
	...
	`	*	VAL_RIGHT_YAXIS	The y-axis on the right side of the graph or strip chart.
		...
		*/
		...
		log = (int)log10(rate);
		SetCtrlAttribute(myApp.hCalibration,PANEL1_GRAPH,ATTR_XPRECISION,log);
		SetAxisScalingMode (myApp.hCalibration,PANEL1_GRAPH, VAL_BOTTOM_XAXIS, VAL_AUTOSCALE, 0.0,1.0);
		...
		TotalSamplesRead = numChannels * numRead; 
...
int BACalibrationFn ()
{
	...
	****/
	...
	plotHandle = PlotX(myApp.hCalibration,PANEL1_GRAPH,dataX_V,(size_t)iNumPts, VAL_DOUBLE,VAL_SCATTER,VAL_EMPTY_SQUARE,VAL_SOLID,
			  1,VAL_YELLOW);
	SetPlotAttribute(myApp.hCalibration,PANEL1_GRAPH,plotHandle, ATTR_PLOT_LG_TEXT,"PhotoDiode");
...
int initialState()
{
	...
	*			as indicated in user interface
	*
	*
	*************************************/ 
	...
	/********
	* Pulse Enable Low starts in low state - although this is active state it should stay low
	* 	Dev1/port0/line0
	*********/
	...
	/*********
	* All other DO enables set to low or inactive 
	**********/
	/****************
	*  Blue Actinic
	*	BlueActEnHigh: DO: Dev1/port0/line2
	*   TCBlueAct: Counter: Dev1/Ctr2
	*	BlueActV: AO: Dev2/ao2
	*
	*****************/
	// Blue Actinic (BA) Radio button Calibration Panel: BlueActEnHigh
	control_value = 0;
	strncpy (chan, "Dev1/port0/line2", sizeof("Dev1/port0/line2"));
	ToggleDigOutFn(control_value,chan);
	
	
	// control behavior tied to UI by value of control button 
	//  toggle function for this control value includes setting AO to zero for control_value =0
	control = PANEL_4_RB_TCBA;
	control_value = 0;
	...
	/****************
	*  Red Actinic
	*	RedActEnHigh: DO:Dev1/port0/line3 
	*   TCRedAct: Counter: Dev1/Ctr0
	*****************/
	// Red Actinic (RA) Radio button Calibration Panel: RedActEnHigh
	strncpy (chan, "Dev1/port0/line3", sizeof("Dev1/port0/line3"));
	ToggleDigOutFn(control_value,chan);
	
	// PS_RA - Ensure power supply off
	PS_RA_Off();
	
	// TCRedAct 
	// control behavior tied to UI by value of control button 
	//  toggle function for this control value includes setting AO to zero for control_value =0
	control = PANEL_4_RB_TCRA;
	control_value = 0;
	ToggleCntrFn(control, control_value);
	...
	*  Blue Pulse
	*	 PulseEnHigh: Dev1/port0/line1
	*	 TCBlueMeas: blue pulse: Dev1/Ctr1 Out	 
	*
	*****************/
	// Blue Pulse (BP) Radio button Calibration Panel: PulseEnHigh
	strncpy (chan, "Dev1/port0/line1", sizeof("Dev1/port0/line1"));
	ToggleDigOutFn(control_value,chan);
	
	// PS_BP - Ensure power supply off
	PS_BP_Off();
	
	// TCBlueMeas (Blue Pulse) Dev1/Ctr1 Out 
	// control behavior tied to UI by value of control button 
	//  toggle function for this control value includes setting AO to zero for control_value =0
	control = PANEL_4_RB_TCBP;
	control_value = 0;
	ToggleCntrFn(control, control_value);
...
//****************************Extra calibration Code*************************/

int Gen_F_SequenceFn (float64 rate, uInt32 sampsPerCycle, int bFirst, int iGraph, int bFm, int bBA_ON)
{
	/************************************
	*  Gen_F_SequenceFn - generate fluorescence sequence function
	*		Stuart Rose
	*		3/16/2016
	*		Notes: Initialize enables to correct values and make sure toggle values start 
	*			as indicated in user interface
	...
	*************************************/
	...
	float64		*temp3=NULL;
	float64		*dataAO=NULL;
	// AO time base: corresponds to timing of AO
	uInt32		sampsPerCycleAO;
	uInt32		TotalsampsPerCycleAO;
	uInt32 		rateAO;
	// multiDimensional interleaved area for AO
	...
	// char        triggerSrc[256];
	int32       numRead;
	uInt32      numChannelsAI;	// Channels of AI
	uInt32		numChannelsAO; 	// Channels of AO
	uInt32      i;
	...
	if( 1 ) {
		...
		****/
		...
		// gTaskHandle one triggers off input at PFI0 which is the output from Dev3/Ctr0Output
		DAQmxErrChk (DAQmxCfgDigEdgeStartTrig (gTaskHandle1, "/Dev2/PFI0", DAQmx_Val_Rising));
		// Set reference voltage info for card - Fixed value of 5.0 hard coded
		DAQmxErrChk (DAQmxSetAODACRefVal(gTaskHandle1,chan, 5.0));
		DAQmxErrChk (DAQmxSetAODACRefSrc(gTaskHandle1,chan,DAQmx_Val_External));
		DAQmxErrChk (DAQmxSetAODACRefExtSrc(gTaskHandle1,chan,"EXTREF"));
		// numChannels = 2  or 3 because this is RA voltage and current AO and possibly BA)
		...
		*
		* AO output time base
		*		rateAO - CfgSampClkTiming (gTaskHandle1: )
		*		sampsPerCycleAO - CfgSampClkTiming (gTaskHandle1:)
		*		TotalsampsPerCycleAO - dataAO (malloc)
		*   
		*	gTaskHandle1: AO output
		*		signals:
		*			BlueActVoltage: BA_Voltage: "Dev2/ao2" 
		*			RA_Voltage "Dev2/ao5" and 
		*			RA_Current "Dev2/ao6"	
		*
		********************************/
		...
		if(bFm){
			...
			// Fill buffer 'data' for generation of red actinic
			// GenSquareWaveRA(int numElementsAO, int numElementsAI,double scaledV, double scaledI, int bBA_ON, double squareWave[], double temp1[], double temp2[], double temp3[])
			RedActScVolt1 = myApp.appCal.calRAControl.RA_Voltage;
			RedActScCurr1 = myApp.appCal.calRAControl.RA_Current;
			...
			ampV = (RedActScVolt1)/12;
		 	ampI = 2*(RedActScCurr1)/5;
			GenSquareWaveRA (TotalsampsPerCycleAO,sampsPerChan, ampV, ampI, baVolt, bBA_ON, dataAO,temp1,temp2,temp3);
		}
		...
		*/ 
		...
		Acquire(rate,sampsPerCycle,&taskHandleAcq);
		// Get number of channedls (numChannels) from task (taskHandleAcq)
		DAQmxErrChk (DAQmxGetTaskAttribute(taskHandleAcq,DAQmx_Task_NumChans,&numChannelsAI));

		// size (malloc) array for acquiring data
		if( (dataAcq=malloc(sampsPerCycle*numChannelsAI*sizeof(float64)))==NULL ) {
			MessagePopup("Error","Not enough memory");
			goto Error;
		} 
		...
		*******************************************************/
		...
		DAQmxErrChk (DAQmxReadAnalogF64(taskHandleAcq,sampsPerCycle,10.0,DAQmx_Val_GroupByChannel,dataAcq,
										sampsPerCycle*numChannelsAI,&numRead,NULL));
	...
	`	*	VAL_RIGHT_YAXIS	The y-axis on the right side of the graph or strip chart.
		...
		*/
		//  Put in multiplier for PAR sensor
		for(int y=0; y<(numChannelsAI * numRead); y++)
		{
			dataAcq[y] = dataAcq[y] * myApp.PARConst;
		}
		...
		//Clear graph
		DeleteGraphPlot(myApp.hCalibration,iGraph,-1,VAL_IMMEDIATE_DRAW);
		
		// Set up XAxis
		// set graph controls for time base
		SetCtrlAttribute(myApp.hCalibration,iGraph,ATTR_XAXIS_GAIN,1/rate);
		log = (int)log10(rate);
		SetCtrlAttribute(myApp.hCalibration,iGraph,ATTR_XPRECISION,log);
		SetAxisScalingMode (myApp.hCalibration,iGraph, VAL_BOTTOM_XAXIS, VAL_AUTOSCALE, 0.0,1.0);

		
		// Set left axis active
		SetCtrlAttribute(myApp.hCalibration,iGraph,ATTR_ACTIVE_YAXIS,VAL_LEFT_YAXIS);
		// SetAxisScalingMode (myCalibration.hCalibration,PANEL1_GRAPH, VAL_LEFT_YAXIS, VAL_MANUAL, 0, 10.0);

		
		SetCtrlAttribute (myApp.hCalibration,iGraph, ATTR_ENABLE_ZOOM_AND_PAN, 1);
		SetCtrlAttribute (myApp.hCalibration,iGraph, ATTR_ZOOM_STYLE, VAL_ZOOM_TO_RECT); 
		
		
		TotalSamplesRead = numChannelsAI * numRead; 
												  
		
		if( numRead>0 )
			for(int j=0;j<numChannelsAI;j++)
			{
				plotHandle = PlotY(myApp.hCalibration,iGraph,&(dataAcq[j*numRead]),numRead,VAL_DOUBLE,VAL_THIN_LINE,VAL_EMPTY_SQUARE,VAL_SOLID,
						  1,VAL_YELLOW);
				SetPlotAttribute(myApp.hCalibration,iGraph,plotHandle, ATTR_PLOT_LG_TEXT,"PhotoDiode");
				PlotY(myApp.hPlotPanel,PANEL_GRAPH,&(dataAcq[j*numRead]),numRead,VAL_DOUBLE,VAL_THIN_LINE,VAL_EMPTY_SQUARE,VAL_SOLID,
					  1,plotColors[j%12]);
			}
		
		// make right axis visible (left visible by default
		SetCtrlAttribute (myApp.hCalibration,iGraph, ATTR_ACTIVE_YAXIS, VAL_RIGHT_YAXIS);
		SetCtrlAttribute (myApp.hCalibration,iGraph, ATTR_YLABEL_VISIBLE, 1);
		...
		*/
		...
		for (int i=0; i<numChannelsAO; i++) {
			if( i==1 ){
				plotHandle = PlotY (myApp.hCalibration, iGraph, &temp3[0], sampsPerCycle,
				   VAL_DOUBLE, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, 1,
				   ColorArray[i]);
				SetPlotAttribute(myApp.hCalibration,iGraph,plotHandle, ATTR_PLOT_LG_TEXT,"RA_Current");
			} else if ( i==2 ) {
				arrayspot = 0;
				plotHandle = PlotY (myApp.hCalibration, iGraph, &temp1[0], sampsPerCycle,
				   VAL_DOUBLE, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, 1,
				   ColorArray[i]);
				SetPlotAttribute(myApp.hCalibration,iGraph,plotHandle, ATTR_PLOT_LG_TEXT,"BA_Voltage");
			} else {
				arrayspot = 0;
				plotHandle = PlotY (myApp.hCalibration, iGraph, &temp2[0], sampsPerCycle,
				   VAL_DOUBLE, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, 1,
				   ColorArray[i]);
				SetPlotAttribute(myApp.hCalibration,iGraph,plotHandle, ATTR_PLOT_LG_TEXT,"RA_Voltage");
			} 
		}
		//TODO: Free all arrays used for generating plots
		/**********Finished plotting Blue Act and Red Act outs****************/
...
int CVICALLBACK Gen_Exp_Sequence (int panel, int control, int event,
								  void *callbackData, int eventData1, int eventData2)
{
	/************************************
	*  Gen_F_Sequence CallBack - generate fluorescence sequence function
	*		Stuart Rose
	*		3/22/2016
	*		Notes: Callback switchboard for controls. Generate sequences of measurements
	*			Initialize enables to correct values and make sure toggle values start 
	*			as indicated in user interface
	*
	*
	*************************************/
	...
	// Functional booleans: identify sequence
	int bFm;					//Is sequence Fm
	...
	// id for graph control on calibration window that is being used
	int iGraph;
	...
	switch (event)		
	{
		case EVENT_COMMIT:
			...
			if (control == PANEL_2_CB_F_FM_GenSeq){
				...
				*		PulseEnHigh DO: "Dev1/port0/line1" (old taskHandle5)
				*		PulseEnLow  DO:  see DO for red actinic below
				*		(Critical timing: Gen_F_SequenceFn: TCBlueMeas: Ctr: "Dev1/Ctr1" out)
				********/  
				...
				*/  
				...
				/************************
				*
				* int bFirst, int iGraph, int bFm, int bBA_ON
				*
				**********/
				// start with not Fm ...just F
				bFm = 0;
				iGraph = PANEL1_GRAPH3;
				
				Gen_F_SequenceFn( rate,sampsPerCycle,0,iGraph,bFm,bBA_ON);
				
				
				
				bFm = 1;							// testing code with bFm first
				iGraph = PANEL1_GRAPH;
				...
				CloseCamera();
								 ...
								 // int Gen_F_SequenceFn (int bFirst, int bLast, int bFm, int bBA_ON)
