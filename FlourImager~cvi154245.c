...
int main (int argc, char *argv[])   
{
	...
	*/
    ...
    errChk (myCalibration.hCalibration = LoadPanel (myCalibration.hParent, "FlourImager.uir", PANEL1));
	myApp.phCalibration = myCalibration.hCalibration; 
    errChk (myCalibration.g_hchild2 = LoadPanel (myCalibration.hParent, "FlourImager.uir", PANEL)); 
	
	//displaying signal plot panel, g_plotpanel
	if((myCalibration.hplotpanel=LoadPanel(0,"Acq-IntClk-DigRef.uir",PANEL))<0 )
			return -1;
	myApp.phPlotPanel = myCalibration.hplotpanel;
	
	SetCtrlAttribute(myCalibration.hplotpanel,PANEL_DECORATION_BLUE,ATTR_FRAME_COLOR,VAL_BLUE);
	SetCtrlAttribute(myCalibration.hplotpanel,PANEL_DECORATION_GREEN,ATTR_FRAME_COLOR,VAL_GREEN);
	SetCtrlAttribute(myCalibration.hplotpanel,PANEL_DECORATION_YELLOW,ATTR_FRAME_COLOR,VAL_YELLOW);
	NIDAQmx_NewPhysChanAICtrl(myCalibration.hplotpanel,PANEL_CHANNEL,1);
	NIDAQmx_NewTerminalCtrl(myCalibration.hplotpanel,PANEL_TRIGSRC,0);

	
    if (myCalibration.hParent   < 0
    ||  myCalibration.hCalibration < 0)
		return -1;  
	
	DisplayPanel (myApp.phCalibration);
	DisplayPanel(myCalibration.hplotpanel); 
	...
	****/
	...
	SetCtrlAttribute (myCalibration.hCalibration, PANEL1_GRAPH_2, ATTR_XFORMAT, VAL_RELATIVE_TIME_FORMAT);
	
	
	// New graph for displaying data collected in burst mode
	//SetAxisScalingMode(myCalibration.hCalibration, PANEL1_GRAPH3, VAL_LEFT_YAXIS, VAL_MANUAL, -15.0,0.5);
    //SetAxisScalingMode(myCalibration.hCalibration, PANEL1_GRAPH3, VAL_BOTTOM_XAXIS, VAL_MANUAL, 0,4096);
	
	//BottomGraph
	//SetAxisScalingMode(myCalibration.hCalibration, PANEL1_GRAPH_3, VAL_LEFT_YAXIS, VAL_MANUAL, 0,100);
    //SetAxisScalingMode(myCalibration.hCalibration, PANEL1_GRAPH_3, VAL_BOTTOM_XAXIS, VAL_MANUAL, 0,1000);
...
Error:
	/* clean up */
	if (myCalibration.hCalibration > 0)
		DiscardPanel (myCalibration.hCalibration);
		DiscardPanel(myCalibration.hplotpanel); 
	return 0;
}
...
static void GenSquareWaveRA(int numElementsAO, int numElementsAI,double scaledV, double scaledI, int bBA_ON, double squareWave[], double temp1[], double temp2[], double temp3[])
{
	...
	long int 	dwCount = 0;
	int			dwLoop = 0;
	DWORD 		m_dwActinicPulseWidth1 = numElementsAO;
	int			pointGain;
							   ...
							   void *callbackData, int eventData1, int eventData2)
{
	...
	TaskHandle	taskHandle4=0,taskHandle5=0;
	TaskHandle 	gTaskHandle=0, gTaskHandle1=0,gTaskHandle2=0;
	...
	uInt32		sampsPerCycleAO,TotalsampsPerCycleAO;
	uInt32 		rateAO;
	...
	if( event==EVENT_COMMIT ) {
		...
		*/
		SetCtrlVal (myCalibration.hCalibration, PANEL1_NUMERICSLIDE, RedActScCurr1);
		
		/************************************
		* Control values necessary for plotting output of this function TurnOnActinic
		*/
		// Controls for getting AI time base info 
		GetCtrlVal(myCalibration.hCalibration,PANEL_SAMPSPERCHAN,&sampsPerCycle);
		GetCtrlVal(myCalibration.hCalibration,PANEL_RATE,&rate); 
		...
		*  Camera setup
		******/
		GetCtrlVal(myCalibration.hCalibration,PANEL1_PICFILE,&(*fileChar)); 
		nMeasPulseWidth = myCalibration.calBPControl.ExposureTime;
		Camera(nMeasPulseWidth); 
		/*******************************************/
		...
		*/ 
		...
		// Don't know
		SetCtrlAttribute(myCalibration.hCalibration,PANEL_ACQUIRE,ATTR_DIMMED,1);
		ProcessDrawEvents();
	...
	`	*	VAL_RIGHT_YAXIS	The y-axis on the right side of the graph or strip chart.
		...
		****/
		...
		SetCtrlAttribute(myCalibration.hCalibration,PANEL1_GRAPH,ATTR_XAXIS_GAIN,1/rate);
		log = (int)log10(rate);
		SetCtrlAttribute(myCalibration.hCalibration,PANEL1_GRAPH,ATTR_XPRECISION,log);
		
		/*********
		*  Graphing AI from photodiode
		*/
		// Reverse the values of the photodiode so it goes from negative to positive
		...
		SetCtrlAttribute(myCalibration.hCalibration,PANEL1_GRAPH,ATTR_ACTIVE_YAXIS,VAL_LEFT_YAXIS);
		// SetAxisScalingMode (myCalibration.hCalibration,PANEL1_GRAPH, VAL_LEFT_YAXIS, VAL_MANUAL, 0, 10.0);

		SetCtrlAttribute(myCalibration.hCalibration,PANEL1_GRAPH,ATTR_XAXIS_GAIN,1.0/rate);
		SetCtrlAttribute(myCalibration.hCalibration,PANEL1_GRAPH,ATTR_XPRECISION,log);
		SetCtrlAttribute (myCalibration.hCalibration,PANEL1_GRAPH, ATTR_ENABLE_ZOOM_AND_PAN, 1);
		SetCtrlAttribute (myCalibration.hCalibration,PANEL1_GRAPH, ATTR_ZOOM_STYLE, VAL_ZOOM_TO_RECT);
		...
		//Clear graph
		DeleteGraphPlot(myCalibration.hCalibration,PANEL1_GRAPH,-1,VAL_IMMEDIATE_DRAW);
		if( numRead>0 )
			for(int j=0;j<numChannels;j++)
			{
				plotHandle = PlotY(myCalibration.hCalibration,PANEL1_GRAPH,&(dataAcq[j*numRead]),numRead,VAL_DOUBLE,VAL_THIN_LINE,VAL_EMPTY_SQUARE,VAL_SOLID,
						  1,VAL_YELLOW);
				SetPlotAttribute(myCalibration.hCalibration,PANEL1_GRAPH,plotHandle, ATTR_PLOT_LG_TEXT,"PhotoDiode");
				PlotY(myCalibration.hplotpanel,PANEL_GRAPH,&(dataAcq[j*numRead]),numRead,VAL_DOUBLE,VAL_THIN_LINE,VAL_EMPTY_SQUARE,VAL_SOLID,
					  1,plotColors[j%12]);
			}
		
		// make right axis visible (left visible by default
		SetCtrlAttribute (myCalibration.hCalibration,PANEL1_GRAPH, ATTR_ACTIVE_YAXIS, VAL_RIGHT_YAXIS);
		SetCtrlAttribute (myCalibration.hCalibration,PANEL1_GRAPH, ATTR_YLABEL_VISIBLE, 1);
							...
							void *callbackData, int eventData1, int eventData2)
{
	...
	//time base: corresponds to timing of AI
	uInt32		sampsPerChannel,TotalSamps;
	uInt32 		sampsPerChan;
	float64		rate; 
	// 1D arrays for holding data to be plotted in AI time base (samples per cycle/ samples per channel)
	float64		*temp1=NULL;
	...
	float64		*dataAO=NULL;
	// time base: corresponds to timing of AO
	uInt32		sampsPerChannelAO,TotalSampsAO;
	uInt32 		rateAO;
	// multiDimensional interleaved area for AO
	float64     *dataAcq=NULL;
	int32 		written;
	...
	if( event==EVENT_COMMIT ) {
		...
		*/
		// Controls for getting AI time base info 
		GetCtrlVal(myCalibration.hCalibration,PANEL_SAMPSPERCHAN,&sampsPerChannel);
		GetCtrlVal(myCalibration.hCalibration,PANEL_RATE,&rate); 
	...
	`	*	VAL_RIGHT_YAXIS	The y-axis on the right side of the graph or strip chart.
		...
		****/
		...
		// Set up XAxis
		// set graph controls for time base
		SetCtrlAttribute(myCalibration.hCalibration,PANEL1_GRAPH3,ATTR_XAXIS_GAIN,1/rate);
		log = (int)log10(rate);
	...
	/*	SetCtrlAttribute(myCalibration.hplotpanel,PANEL_GRAPH,ATTR_XAXIS_GAIN,1.0/rate);
		...
		*/
		...
		for (int i=0; i<numChannelsAO; i++) {
			if( i==1 ){
				plotHandle = PlotY (myCalibration.hCalibration, PANEL1_GRAPH, &temp3[0], sampsPerChannel,
				   VAL_DOUBLE, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, 1,
				   ColorArray[i]);
				SetPlotAttribute(myCalibration.hCalibration,PANEL1_GRAPH,plotHandle, ATTR_PLOT_LG_TEXT,"RA_Current");
			} else if ( i==2 ) {
				arrayspot = 0;
				plotHandle = PlotY (myCalibration.hCalibration, PANEL1_GRAPH, &temp1[0], sampsPerChannel,
				   VAL_DOUBLE, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, 1,
				   ColorArray[i]);
				SetPlotAttribute(myCalibration.hCalibration,PANEL1_GRAPH,plotHandle, ATTR_PLOT_LG_TEXT,"BA_Voltage");
			} else {
				arrayspot = 0;
				plotHandle = PlotY (myCalibration.hCalibration, PANEL1_GRAPH, &temp2[0], sampsPerChannel,
				   VAL_DOUBLE, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, 1,
				   ColorArray[i]);
				SetPlotAttribute(myCalibration.hCalibration,PANEL1_GRAPH,plotHandle, ATTR_PLOT_LG_TEXT,"RA_Voltage");
			} 
							...
							void *callbackData, int eventData1, int eventData2)
{
	...
	TaskHandle  taskHandle1=0,taskHandle2=0,taskHandle3=0;
	TaskHandle	taskHandle5=0;
	TaskHandle 	gTaskHandle=0, gTaskHandle2=0;
	...
	if( event==EVENT_COMMIT ) {
		...
		*/
		// Controls for getting AI time base info 
		GetCtrlVal(myCalibration.hCalibration,PANEL1_SAMPSPERCHAN_2,&sampsPerCycle);
		GetCtrlVal(myCalibration.hCalibration,PANEL1_RATE_2,&rateAI); 
		...
		*/
		...
		DAQmxErrChk (DAQmxCreateTask("",&gTaskHandle));
		// negative pulse at time zero 
		double initDelay,lowTime,highTime; 
	...
	`	*	VAL_RIGHT_YAXIS	The y-axis on the right side of the graph or strip chart.
		...
		*/
		...
		//Clear graph
		DeleteGraphPlot(myCalibration.hCalibration,PANEL1_GRAPH3,-1,VAL_IMMEDIATE_DRAW);
		SetCtrlAttribute (myCalibration.hCalibration,PANEL1_GRAPH3, ATTR_ENABLE_ZOOM_AND_PAN, 1);
		SetCtrlAttribute (myCalibration.hCalibration,PANEL1_GRAPH3, ATTR_ZOOM_STYLE, VAL_ZOOM_TO_RECT);		
		// Set left axis active
		// SetCtrlAttribute(myCalibration.hCalibration,PANEL1_GRAPH3,ATTR_ACTIVE_YAXIS,VAL_LEFT_YAXIS);
		SetAxisScalingMode (myCalibration.hCalibration,PANEL1_GRAPH, VAL_LEFT_YAXIS, VAL_AUTOSCALE, 0, 5.0);
		...
		if( numRead>0 )
			for(int j=0;j<numChannels;j++)
			{
				plotHandle = PlotY(myCalibration.hCalibration,PANEL1_GRAPH3,&(dataAcq[j*numRead]),numRead,VAL_DOUBLE,VAL_THIN_LINE,VAL_EMPTY_SQUARE,VAL_SOLID,
						  1,VAL_YELLOW);
				SetPlotAttribute(myCalibration.hCalibration,PANEL1_GRAPH3,plotHandle, ATTR_PLOT_LG_TEXT,"PhotoDiode");
				PlotY(myCalibration.hplotpanel,PANEL_GRAPH,&(dataAcq[j*numRead]),numRead,VAL_DOUBLE,VAL_THIN_LINE,VAL_EMPTY_SQUARE,VAL_SOLID,
					  1,plotColors[j%12]);
			}
						  ...
						  void *callbackData, int eventData1, int eventData2)
{
		if( event==EVENT_COMMIT )
		{
			GetCtrlVal (myCalibration.hCalibration, PANEL1_NUMERICSLIDE, &myCalibration.calRAControl.RA_Current);	
			SetTableCellVal (myCalibration.hCalibration, PANEL1_TABLE_2, MakePoint(1,2), myCalibration.calRAControl.RA_Current);
		}
						  ...
						  void *callbackData, int eventData1, int eventData2)
{
		if( event==EVENT_COMMIT )
		{
			GetCtrlVal (myCalibration.hCalibration, PANEL1_NUMERICSLIDE_4, &myCalibration.calRAControl.RA_Voltage);	
			SetTableCellVal (myCalibration.hCalibration, PANEL1_TABLE_2, MakePoint(1,3), myCalibration.calRAControl.RA_Voltage);
		}
	return 0;	
}
						  ...
						  void *callbackData, int eventData1, int eventData2)
{
		if( event==EVENT_COMMIT )
		{
			GetCtrlVal (myCalibration.hCalibration, PANEL1_NUMERICSLIDE_3, &myCalibration.calBPControl.BP_Current);	
			SetTableCellVal (myCalibration.hCalibration, PANEL1_TABLE , MakePoint(1,4), myCalibration.calBPControl.BP_Current);
		}
						  ...
						  void *callbackData, int eventData1, int eventData2)
{
	if( event==EVENT_COMMIT )
	{
		GetCtrlVal (myCalibration.hCalibration, PANEL1_NUMERICSLIDE_2, &myCalibration.calBPControl.BP_Voltage);	
		SetTableCellVal (myCalibration.hCalibration, PANEL1_TABLE , MakePoint(1,5), myCalibration.calBPControl.BP_Voltage);
	}
	return 0;	
}
...
void InitializeTablesAndHandles(void)
{
	...
	GetCtrlVal(myCalibration.hCalibration,PANEL1_PICFILE,myCalibration.pathName);   
	...
	****/
	...
	for(int row=1;row<=3;row++){
		for(int col=1;col<=3;col++){
			GetTableCellVal (myCalibration.hCalibration, PANEL1_TABLE, MakePoint(col,row), value_exposure);
	...
	GetTableCellVal (myCalibration.hCalibration, PANEL1_TABLE, MakePoint(1,5), value1);
	myBPControl.BP_Voltage =  *value1;
	
	GetTableCellVal (myCalibration.hCalibration, PANEL1_TABLE, MakePoint(1,4), value1);
	myBPControl.BP_Current =  *value1;
	
	GetTableCellVal (myCalibration.hCalibration, PANEL1_TABLE, MakePoint(1,6), value_exposure);
	...
	int row = 1;
	for(int col=1;col<=3;col++){
		GetTableCellVal (myCalibration.hCalibration, PANEL1_TABLE_2, MakePoint(col,row), value_exposure_x);
	...
	GetTableCellVal (myCalibration.hCalibration, PANEL1_TABLE_2, MakePoint(1,2), value_RA);   
	myCalibration.calRAControl.RA_Current = *value_RA;
	
	GetTableCellVal (myCalibration.hCalibration, PANEL1_TABLE_2, MakePoint(1,3), value_RA);   
	myCalibration.calRAControl.RA_Voltage = *value_RA;
	
	GetTableCellVal (myCalibration.hCalibration, PANEL1_TABLE_2, MakePoint(1,4), value_exposure_x);
	...
	****/
	SetCtrlVal (myCalibration.hCalibration, PANEL1_NUMERICSLIDE, myCalibration.calRAControl.RA_Current);
	SetCtrlVal (myCalibration.hCalibration, PANEL1_NUMERICSLIDE_3, myCalibration.calBPControl.BP_Current);
	SetCtrlVal (myCalibration.hCalibration, PANEL1_NUMERICSLIDE_2, myCalibration.calBPControl.BP_Voltage);
	SetCtrlVal (myCalibration.hCalibration, PANEL1_NUMERICSLIDE_4, myCalibration.calRAControl.RA_Voltage);
	/****************/
