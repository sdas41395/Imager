...
//	}
	...
	*/
	...
	for (int i=1; i<=dwValidImageCnt; i++) 
	{
		...
		*/
		...
		/***** Debugging - printing out time stamps****/
		/*
	...
	**********/
	...
	if (dwValidImageCnt > 0){
		...
		//	MessagePopup("Display and Saving tif file name",file_tif);
			strcpy(myData->images[i2]->Image_PathName, file_tif);
	...
	*/
	...
	//Number that the campicutre has been called and adding it to the array to be pointe
	...
	//the above is the position in the table. 
	myApp.CExperimentDataCount++;  //  = CExperimentDataCount++;
	// insert row for this newly stored CExperimentData struct
	InsertTableRows(myApp.hExperiment,myApp.dataTableCtrl,-1,myApp.CExperimentDataCount,VAL_USE_MASTER_CELL_TYPE);
	//inserting information into created cells from the data struct.
	
	//Setting sequencenumber
	char name[100];
	sprintf(name,"Sequence Number: %d",myApp.CExperimentDataCount);
	SetTableCellVal (myApp.hExperiment,myApp.dataTableCtrl, MakePoint(1,myApp.CExperimentDataCount),name);
	//Setting buttons and ??deleting?? -row.
	
	// look at initialization to see that column type set to button.
	char delete[20];
	sprintf(delete,"Delete");
	SetTableCellAttribute (myApp.hExperiment, myApp.dataTableCtrl,
                       MakePoint(4,myApp.CExperimentDataCount), ATTR_CELL_TYPE,
                       VAL_CELL_BUTTON);
	SetTableCellVal(myApp.hExperiment,myApp.dataTableCtrl,MakePoint(4,myApp.CExperimentDataCount),delete);
	...
	SetTableCellAttribute (myApp.hExperiment, myApp.dataTableCtrl,
                       MakePoint(3,myApp.CExperimentDataCount), ATTR_CELL_TYPE,
                       VAL_CELL_BUTTON); 
	SetTableCellVal(myApp.hExperiment,myApp.dataTableCtrl,MakePoint(3,myApp.CExperimentDataCount),display);
	
	//Connection to the callback.
	InstallCtrlCallback(myApp.hExperiment, myApp.dataTableCtrl, Data_Table, 0);
	
	//TimeStamp Implementation into table - 
	...
	SetTableCellVal(myApp.hExperiment,myApp.dataTableCtrl,MakePoint(2,myApp.CExperimentDataCount),TimeStamp);
	...
	SetRowHeightToTallestCellContents(myApp.hExperiment, myApp.dataTableCtrl,myApp.CExperimentDataCount);  
	SetTableCellAttribute (myApp.hExperiment, myApp.dataTableCtrl,
                       MakePoint(1,myApp.CExperimentDataCount), ATTR_WRAP_MODE, VAL_CHAR_WRAP);
	SetTableCellAttribute (myApp.hExperiment, myApp.dataTableCtrl,
                       MakePoint(2,myApp.CExperimentDataCount), ATTR_WRAP_MODE, VAL_CHAR_WRAP);
	SetTableCellAttribute (myApp.hExperiment, myApp.dataTableCtrl,
                       MakePoint(3,myApp.CExperimentDataCount), ATTR_WRAP_MODE, VAL_CHAR_WRAP);
	SetTableCellAttribute (myApp.hExperiment, myApp.dataTableCtrl,
                       MakePoint(4,myApp.CExperimentDataCount), ATTR_WRAP_MODE, VAL_CHAR_WRAP);
	for(int i=1; i<5; i++){
	...
	SetRowHeightToTallestCellContents (myApp.hExperiment,
                                    myApp.dataTableCtrl, myApp.CExperimentDataCount);
	}

	
	//Eliminate multiple shut downs of power supplies try to keep control
	// PS_BP_Off();
	// PS_RA_Off();
	...
	***********************************************/
	...
	if( event==EVENT_COMMIT){
		...
		/* eventData2 = column of cell where event was generated; if 0, event affected multiple cells */
			...
			//This Displays the data at the third event which is the display button
				...
				iRetCode = imaqSetImageSize(image,myApp.wXResAct, myApp.wYResAct);
				iRetCode = imaqArrayToImage(image,tempData.images[i2].meta_image,myApp.wXResAct, myApp.wYResAct);   //taking the image from FI_Image
				if (iRetCode == ERR_SUCCESS) {
					error_code = imaqGetLastError();
					strcpy(errBuff, imaqGetErrorText(error_code));
					sprintf(strTemp,"imaq Error code %lx\n And Returned erro code:  %lx\nAnd text out: %c", error_code, iRetCode,*errBuff);
			...
			if(eventData1 != 0 && eventData2 == 4){
				...
				for(int i2 = 0; i2<5; i2++){
					free(myApp.CExperimentData_Array[eventData1-1].images[i2].meta_image);
				}
...
void InitializeTablesAndHandles(void)
{
	...
	******/
	...
	// initialize a singular instance of CExperimentData?  Why?
	CExperimentData *myData;
	...
	********/
	...
	SetTableColumnAttribute (myApp.hExperiment,
							 dataTableCtrl,1,
							 ATTR_LABEL_TEXT, "Sequence Number");
	...
	//SetTableColumnAttribute(myApp.hExperiment,dataTableCtrl,
