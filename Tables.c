...
{
	/*****************************************
	* Title:  	BP_Table (CALLBACK)
	* Author: 	Shoham Das and Stuart Rose
	* Date: 	Rewritten 4/19/2016
	* Note: 	Initial setting for instrument triggers and BP stored in this table and this function
	*			responds to change (commit) events in table
	*
	***********************************************/
	...
	if( event==EVENT_COMMIT ){
		...
		*/
		...
		if(eventData1 != 0 & eventData2 != 0){
			...
			if(eventData1 == 1){
			/*************
			* First Trigger (Row,Col) Experiment trigger
			*		(1,1)variable: Calibration.iTrigInitDelay: triggering pulse (Dev2/Ctr0?)
			*		(1,2)variable: Calibration.iTrigLow 
			*		(1,3)variable: Calibration.iTrigHigh = high part of cycle
			*
			**/
				...
				if(eventData2 == 1)
				{
					GetTableCellVal (myApp.hCalibration, PANEL1_TABLE, MakePoint(eventData1,eventData2), &table_int);
					if(table_int < 0)
					{
						sprintf(strTemp,"Out of bounds(HB):.%d\n Old value returned: &d\n ", table_int,myCalibration.iTrigInitDelay);
						MessagePopup("BP Table Main Trigger setting",strTemp);
						SetTableCellVal (myApp.hCalibration, PANEL1_TABLE,MakePoint(eventData1,eventData2),myCalibration.iTrigInitDelay);		
					}
					else{
						myCalibration.iTrigInitDelay = table_int;
			...
			if(eventData1 == 2){
			/*************
			* Blue Pulse (Row,Col) Blue Pulse(BP or measuring pulse) 
			*		(2,1)variable: BPControl.iBP_InitDelay (int) = setting of initial BP delay
			*		(2,2)variable: BPControl.iBP_Low (int) = low part of cycle
			*		(2,3)variable: BPControl.iBP_High (int) = high part of cycle
			*
			**/
				...
				if(eventData2 == 1)
				{
					...
					if(table_int < 0)
					{
						// bounds check - set back to old value
						sprintf(strTemp,"Out of bounds(HB):.%d\n Old value returned: &d\n ", table_int,myBPControl.iBP_InitDelay);
						MessagePopup("BP Table Pulse setting",strTemp);
						SetTableCellVal (myApp.hCalibration, PANEL1_TABLE,MakePoint(eventData2,eventData1),myBPControl.iBP_InitDelay);
					}
					else{
						// change value in struct to value found in table
						myBPControl.iBP_InitDelay = table_int;	
				...
				if(eventData2 == 2)
				{
					...
					{
						// bounds check - set back to old value
						sprintf(strTemp,"Out of bounds(HB):.%d\n Old value returned: &d\n ", table_int,myBPControl.iBP_Low);
						MessagePopup("BP Table Pulse setting",strTemp);
						SetTableCellVal (myApp.hCalibration, PANEL1_TABLE,MakePoint(eventData1,eventData2),myBPControl.iBP_Low);
					}
					else{
						// change value in struct to value found in table
						myBPControl.iBP_Low = table_int;	
					}
				}
				...
				if(eventData2 == 3)
				{
					...
					{
						// bounds check - set back to old value
						sprintf(strTemp,"Out of bounds(HB):.%d\n Old value returned: &d\n ", table_int,myBPControl.iBP_High);
						MessagePopup("BP Table Pulse setting",strTemp);
						SetTableCellVal (myApp.hCalibration, PANEL1_TABLE,MakePoint(eventData2,eventData1),myBPControl.iBP_High);
					}
					else{
						// change value in struct to value found in table
						myBPControl.iBP_High = table_int;	
			...
			*		(3,1)variable: BPControl.iCamTrigInitDelay (int) = setting of initial trigger delay
			*		(3,2)variable: BPControl.iCamTrigLowTime (int) = low part of cycle
			*		(3,3)variable: BPControl.iCamTrigHighTime (int) = high part of cycle
			*
			**/
				...
				if(eventData2 == 1)
				{
					...
					if(table_int < 0)
					{
						sprintf(strTemp,"Out of bounds(HB):.%d\n Old value returned: &d\n ", table_int,myBPControl.iCamTrigInitDelay);
						...
						myBPControl.iCamTrigInitDelay = table_int;	
					}
				}
				...
				if(eventData2 == 3)
				{
					...
					if(table_int < 0)
					{
						sprintf(strTemp,"Out of bounds(HB):.%d\n Old value returned: &d\n ", table_int,myBPControl.iCamTrigHighTime);
						MessagePopup("BP Table CamTrig setting",strTemp);
						SetTableCellVal (myApp.hCalibration, PANEL1_TABLE,MakePoint(eventData2,eventData1),myBPControl.iCamTrigHighTime);
					}
					else{
						myBPControl.iCamTrigHighTime = table_int;	
						SetTableCellVal(myApp.hCalibration,PANEL1_TABLE, MakePoint(eventData2, eventData1), myBPControl.iCamTrigHighTime);
			...
			**/
				...
				if(eventData2 == 1)
				{
					...
					//Retrieving bounds
					bounds_low = myBPControl.BP_I_LB;
					bounds_high = myBPControl.BP_I_HB;
				...
				{
					GetTableCellVal (myApp.hCalibration, PANEL1_TABLE, MakePoint(eventData2,eventData1), &table_double); 
					myBPControl.BP_I_HB = table_double;
				}
			}
			...
			**/
				...
				{
					GetTableCellVal (myApp.hCalibration, PANEL1_TABLE, MakePoint(eventData2,eventData1), &table_double); 
					myBPControl.BP_V_HB = table_double;
				}
			}
			
			// Column 6, Row 1	
			if(eventData1 == 6 && eventData2 == 1){
			/*************
			...
			**/
				if(eventData2 == 1)
				{
					...
					if(table_int < 0 || table_int > 1000)
					{
						sprintf(strTemp,"Out of bounds(HB):.%d\n Old value returned: &d\n ", table_int,myBPControl.ExposureTime);
						MessagePopup("BP Table ExposureTime setting",strTemp);
						SetTableCellVal (myApp.hCalibration, PANEL1_TABLE,MakePoint(eventData2,eventData1),myBPControl.ExposureTime);
					}
					else{
						myBPControl.ExposureTime = table_int;	
						SetTableCellVal(myApp.hCalibration,PANEL1_TABLE, MakePoint(eventData2, eventData1), myBPControl.ExposureTime);
					}
				}
			}
